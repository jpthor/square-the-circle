...
def completions(objkeys, pts, depth, budget, collect):
    ...
    # ---- axis transfer completions ----
    axis_pts = [p for p in P if abs(p[1]) < 1e-9]
    axis_vals = {}   # rounded coord -> (val, cost, source)
    for (axp, _) in axis_pts:
        axis_vals.setdefault(round(axp, RND), (axp, 0, ('pt', axp)))
    drawn_c = set((round(o[1], RND), round(o[2], RND), round(o[3], RND)) for o in circles)
    for (axp, _) in axis_pts:
        for rk, (rv, rcost, rsrc) in rho_pool.items():
            ccost = 0 if (round(axp, RND), 0.0, rk) in drawn_c else 1
            for sg in (1.0, -1.0):
                v = axp + sg*rv
                if abs(v) > 4.4: continue
                k = round(v, RND)
                cur = axis_vals.get(k)
                if cur is None or cur[1] > ccost:
                    axis_vals[k] = (v, ccost, ('xfer', axp, sg, rsrc))
    av = sorted(axis_vals.values(), key=lambda t: t[0])
    avx = np.array([t[0] for t in av])
    avc = np.array([t[1] for t in av], dtype=int)
    # (i) axis value vs any state point V
    for (vxx, vyy) in P:
        fcost = 0 if abs(vyy) < 1e-9 else 1
        base = depth + avc + fcost
        s2 = (avx - vxx)**2 + vyy*vyy
        err = np.abs(s2 - PI)
        feas = base <= budget
        if feas.any():
            best = min(best, float(err[feas].min()))
        for idx in np.where((err < LOG_NEAR) & feas)[0]:
            collect.append((float(err[idx]), int(base[idx]), 'AXFER',
                            (av[idx][2], 'V', (vxx, vyy))))
    # (ii) axis value vs axis value (free final along L0)
    SP = math.sqrt(PI)
    m = len(avx)
    for tgt in (SP, -SP):
        pos = np.searchsorted(avx, avx + tgt)
        for i in range(m):
            for j in (pos[i]-1, pos[i], pos[i]+1):
                if 0 <= j < m and j != i:
                    s = abs(avx[i] - avx[j])
                    e = abs(s*s - PI)
                    cost = depth + int(avc[i] + avc[j])
                    if cost <= budget:
                        best = min(best, e)
                        if e < LOG_NEAR:
                            collect.append((e, cost, 'AXAX', (av[i][2], av[j][2])))
...
def main():
    ...
    def show(hits, header, limit):
        print(header)
        seen = set(); shown = 0
        for err, cost, depth, ck, kind, info in hits:
    ...
    show(sorted(all_hits, key=lambda h: (h[1], h[0])), '\n==== hits by (cost, err) ====', 25)
    show(sorted(all_hits, key=lambda h: h[0]), '\n==== hits by err ====', 25)
    return
